...
namespace PurrNet
{
    ...
    public enum NetworkForceMode : byte
    {
        Force = 0,
        Impulse = 1,
        VelocityChange = 2,
    ...
    internal struct RigidbodyCorrectionData
    {
    ...
    internal interface IRigidbodyCorrectionInstance
    {
        bool ShouldTeleport(in RigidbodyCorrectionData data);
        bool ShouldSnapRotation(in RigidbodyCorrectionData data);
        bool ShouldCorrectRotation(in RigidbodyCorrectionData data);
        void ApplyHardCorrection(in RigidbodyCorrectionData data);
        void ApplyPositionCorrection(in RigidbodyCorrectionData data);
        void ApplyRotationCorrection(in RigidbodyCorrectionData data);
        void OnReset(in RigidbodyCorrectionData data);
        void OnDespawned();
    }

    public abstract partial class NetworkRigidbodyBase : NetworkIdentity, ITick
    {
        [Header("Authority")]
...
#pragma warning restore 0414
        ...
        private IRigidbodyCorrectionInstance _settingsInstance;
        private UnityEngine.Object _settingsInstanceSource;
        ...
        protected abstract Component bodyComponent { get; }
        protected abstract Vector3 bodyPosition { get; set; }
        protected abstract Quaternion bodyRotation { get; set; }
        protected abstract Vector3 bodyLinearVelocity { get; set; }
        ...
        protected abstract bool bodyIsSleeping { get; }
        protected abstract bool bodyHasFrozenRotation { get; }
        protected abstract bool canApplyDynamicMotion { get; }
        protected abstract void MoveBodyPosition(Vector3 position);
        protected abstract void MoveBodyRotation(Quaternion rotation);
        protected abstract void AddBodyForce(Vector3 force, NetworkForceMode mode);
        protected abstract void AddBodyForceAtPosition(Vector3 force, Vector3 position, NetworkForceMode mode);
        protected abstract void AddBodyTorque(Vector3 torque, NetworkForceMode mode);
        protected abstract void ApplyBodyPositionSpring(Vector3 targetPosition, Vector3 targetLinearVelocity, float positionError, float positionStrength, float correctionRange);
        protected abstract void ApplyBodyRotationSpring(Quaternion targetRotation, Vector3 targetAngularVelocity, float rotationStrength, bool kinematic);
        protected abstract RigidbodySettingsData GetCurrentSettings();
        protected abstract void ApplySettings(in RigidbodySettingsData settings);
        protected abstract bool TryGetParentBody(Transform parent, out Transform bodyTransform, out Vector3 position, out Quaternion rotation);
        protected abstract bool TryGetParentBodyMotion(Transform parent, out Vector3 linearVelocity, out Vector3 angularVelocity, out Vector3 worldCenterOfMass);
        protected abstract void ClearParentBodyCache();
        protected abstract UnityEngine.Object settingsOverrideAsset { get; }
        internal abstract IRigidbodyCorrectionInstance CreateSettingsInstance();
        internal abstract void RaiseTeleportCorrection(in RigidbodyCorrectionData data);

        protected bool hasBody => bodyComponent;
        ...
        protected void EnsureSettingsInstance()
        {
            var asset = settingsOverrideAsset;
            if (!asset)
            {
            ...
            if (_settingsInstance != null && _settingsInstanceSource == asset)
                return;

            DisposeSettingsInstance();
            _settingsInstance = CreateSettingsInstance();
            _settingsInstanceSource = asset;
        }

        protected void DisposeSettingsInstance()
        {
            if (_settingsInstance == null)
                return;
        ...
        private void FixedUpdate()
        {
            ...
            var ctx = BuildCorrectionData(worldTargetPos, worldTargetRot, worldTargetLinVel, worldTargetAngVel, positionError, rotationError);
            ...
            if (_settingsInstance != null)
            {
                if (_settingsInstance.ShouldTeleport(in ctx))
                {
                    ...
                    RaiseTeleportCorrection(in ctx);
                    return;
                }
                ...
                if (hardSnapRotation)
                {
                    ...
                    MoveBodyRotation(NormalizeQuaternion(worldTargetRot));
        ...
        private bool TryStallSnapRecovery(in RigidbodyCorrectionData ctx)
        {
            ...
            bool bodySettled = (GetLinearVelocity() - ctx.targetLinearVelocity).sqrMagnitude <= SETTLED_LINEAR_SPEED_SQR
                            && (bodyAngularVelocity - ctx.targetAngularVelocity).sqrMagnitude <= SETTLED_ANGULAR_SPEED_SQR;
        ...
        private void ApplySettleServo(in RigidbodyCorrectionData ctx, bool rotationRecovery)
        {
            ...
            var angularVelocity = bodyAngularVelocity;
            ...
            var toTarget = ctx.targetPosition - bodyPosition;
            ...
            if (!rotationDone)
            {
                var rotationDelta = NormalizeQuaternion(ctx.targetRotation) * Quaternion.Inverse(bodyRotation);
        ...
        private bool UpdateRotationStallRecovery(in RigidbodyCorrectionData ctx)
        {
        ...
        private void ApplyCorrection(Vector3 worldTargetPos, Quaternion worldTargetRot, Vector3 worldTargetLinVel, Vector3 worldTargetAngVel, float positionError, bool correctRotation)
        {
            ...
            ApplyBodyPositionSpring(worldTargetPos, worldTargetLinVel, positionError, _positionStrength, _correctionRange);

            if (correctRotation)
                ApplyBodyRotationSpring(NormalizeQuaternion(worldTargetRot), worldTargetAngVel, _rotationStrength, useKinematicRotationCorrection);
        }
            ...
            get
            {
                switch (_rotationCorrection)
                {
                    ...
                    case RigidbodyRotationCorrection.Torque:
                        return false;
                    default:
                        return hasBody && bodyHasFrozenRotation;
                }
            }
        }

        private RigidbodyCorrectionData BuildCorrectionData(Vector3 worldTargetPos, Quaternion worldTargetRot, Vector3 worldTargetLinVel, Vector3 worldTargetAngVel, float positionError, float rotationError)
        {
            return BuildCorrectionData(
                worldTargetPos,
                worldTargetRot,
                worldTargetLinVel,
                worldTargetAngVel,
        ...
        private RigidbodyCorrectionData BuildCorrectionData(
        ...
        {
            return new RigidbodyCorrectionData
            {
                body = bodyComponent,
        ...
        {
            bodyPosition = worldTargetPos;
            bodyRotation = NormalizeQuaternion(worldTargetRot);
            SetLinearVelocity(_resetLinearVelocityOnSnap ? Vector3.zero : worldTargetLinVel);
            SetAngularVelocity(_resetAngularVelocityOnSnap ? Vector3.zero : worldTargetAngVel);
        }

        private static Quaternion NormalizeQuaternion(Quaternion q) => NetworkRigidbodyMath.NormalizeQuaternion(q);
        ...
        public void SetSoftParent(NetworkIdentity identity)
        {
            ...
            if (isSpawned && hasBody && isActiveAndEnabled)
                SendCurrentState(true, IsSettledForSync());
        }
        ...
        {
            if (!TryGetParentBody(parent, out var bodyTransform, out var parentBodyPosition, out var parentBodyRotation))
            {
            ...
            if (bodyTransform == parent)
            {
                position = parentBodyPosition;
                rotation = parentBodyRotation;
                return;
            }

            var localRotation = Quaternion.Inverse(bodyTransform.rotation) * parent.rotation;
            var localPosition = bodyTransform.InverseTransformPoint(parent.position);
            rotation = parentBodyRotation * localRotation;
            position = parentBodyPosition + parentBodyRotation * Vector3.Scale(bodyTransform.lossyScale, localPosition);
        }
        ...
        {
            var p = hasBody ? bodyPosition : transform.position;
            if (parent)
        ...
        {
            var r = hasBody ? bodyRotation : transform.rotation;
        ...
        private Vector3 ReadLinearVelocity(Transform parent)
        {
            ...
            return NetworkRigidbodyFrameMath.ToParentLinearVelocity(
                v,
                GetParentPointVelocity(parent, bodyPosition),
                parentRotation,
                parent.lossyScale);
        }
        ...
        private float GetPositionError(Vector3 worldTargetPos)
        {
            ...
            {
                Vector3 localCurrent = ParentInverseTransformPoint(_targetParent, bodyPosition);
                return Vector3.Distance(localCurrent, ToV3(_targetPosition));
            }

            return Vector3.Distance(bodyPosition, worldTargetPos);
        }
        ...
        {
            if (!parent || !TryGetParentBodyMotion(parent, out var linear, out var angular, out var centerOfMass))
                return Vector3.zero;

            return NetworkRigidbodyFrameMath.GetPointVelocity(linear, angular, centerOfMass, worldPoint);
        }

        private Vector3 GetParentAngularVelocity(Transform parent)
        {
            return parent && TryGetParentBodyMotion(parent, out _, out var angular, out _)
                ? angular
                : Vector3.zero;
        }

        private Vector3 GetLinearVelocity() => bodyLinearVelocity;

        private void SetLinearVelocity(Vector3 value) => bodyLinearVelocity = value;

        private void SetAngularVelocity(Vector3 value) => bodyAngularVelocity = value;

        private bool CanApplyDynamicMotion() => canApplyDynamicMotion;
        ...
        private bool IsSettledForSync()
        {
            ...
            if (bodyIsKinematic || bodyIsSleeping)
                return true;
        ...
        /// </summary>
        protected void SyncSettingsIfChanged()
        {
            if (!hasBody)
                return;
        ...
        private static bool SettingsEqual(in RigidbodySettingsData a, in RigidbodySettingsData b)
        {
            return a.mass.rawValue == b.mass.rawValue
                ...
                && a.gravityScale.rawValue == b.gravityScale.rawValue
        ...
        internal IRigidbodyCorrectionInstance settingsInstanceInternal => _settingsInstance;
        ...
        {
            get => hasBody ? bodyAngularDrag : 0f;
            set
            {
                if (!hasBody)
                    return;
                bodyAngularDrag = value;
                SyncSettingsIfChanged();
            }
        }
        ...
        public bool isKinematic
        {
            get => hasBody && bodyIsKinematic;
            set
            {
                if (!hasBody)
                    return;
                bodyIsKinematic = value;
                SyncSettingsIfChanged();
            }
        }

        protected void AddForceInternal(Vector3 force, NetworkForceMode mode)
        {
            if (!isSpawned || !hasBody)
                return;

            if (IsController(_ownerAuth))
                AddBodyForce(force, mode);
            else if (isActiveAndEnabled)
                BroadcastForce(new AppliedForce { force = force, mode = mode });
        }

        protected void AddForceAtPositionInternal(Vector3 force, Vector3 position, NetworkForceMode mode)
        {
            if (!isSpawned || !hasBody)
                return;

            if (IsController(_ownerAuth))
                AddBodyForceAtPosition(force, position, mode);
            else if (isActiveAndEnabled)
                BroadcastForce(EncodeForceAtPosition(force, position, mode));
        }

        protected void AddTorqueInternal(Vector3 torque, NetworkForceMode mode)
        {
            if (!isSpawned || !hasBody)
                return;

            if (IsController(_ownerAuth))
                AddBodyTorque(torque, mode);
            else if (isActiveAndEnabled)
                BroadcastForce(new AppliedForce { force = torque, mode = mode, isTorque = true });
        }
        ...
        public void TeleportTo(Vector3 position)
        {
            if (!hasBody)
                return;
            TeleportTo(position, bodyRotation);
        }
        ...
        public void TeleportLocal(Vector3 position)
        {
            if (!hasBody)
                return;
            TeleportLocal(position, bodyRotation);
        }
        ...
        private void SendInitialStateToObserver(PlayerID player, RigidbodyStateData data, RigidbodySettingsData settings)
        {
            ...
            bodyPosition = worldPos;
            bodyRotation = NormalizeQuaternion(ToWorldRotation(data.rotation, parentTrs));
